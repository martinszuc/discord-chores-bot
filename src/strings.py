...
class BotStrings:
    ...
    REACTION_INSTRUCTIONS = (
        ...
        "🔄 - Claim yuh did complete it last week but forget fi confirm"
    )
    ...
    EMBED_REACTIONS_GUIDE = (
        ...
        "🔄 - Mi did complete it last week but forget fi confirm"
    )
    ...
    # Pre-week reminder (1 day before new schedule)
    PRE_WEEK_REMINDER_HEADER = "⚠️ **Last Day Reminder - New Week Start Tomorrow!** ⚠️"
    PRE_WEEK_REMINDER_MESSAGE = "Yow {mention}! Yuh still have **{chore}** fi finish before tomorrow. Mek sure yuh complete it, or yuh gonna get extra work next week - real talk!"
    PRE_WEEK_REMINDER_ALL_DONE = "🎉 Everybody done complete dem chores! Irie work, whole crew! 🎉"

    # Penalty system for incomplete tasks
    PENALTY_HEADER = "⚠️ **Penalty Time - Yuh Nuh Finish Last Week!** ⚠️"
    PENALTY_MESSAGE = "{mention}, yuh nuh complete yuh chore **{chore}** last week, so now yuh get extra work fi not being a good teammate. Dis week yuh have: **{new_chore}** (plus yuh regular work). Step up yuh game, bredrin!"
    PENALTY_LUCKY_PERSON = "🎉 {mention}, yuh get a break dis week! Somebody else nuh complete dem work, so yuh get one less task. Enjoy di break! 🏖️"
    PENALTY_NO_TASKS_THIS_WEEK = "🎉 {mention}, yuh get di whole week off! Somebody else nuh complete dem work, so yuh nuh have no tasks dis week. Lucky yuh! 🏖️"

    # Forgot to confirm reaction
    FORGOT_TO_CONFIRM_MESSAGE = "✅ {mention} say dem did complete **{chore}** last week but forget fi confirm. Wi update di records - bless up!"
    FORGOT_TO_CONFIRM_TOO_LATE = "❌ {mention}, too late fi claim yuh did complete **{chore}**. Di new schedule already post. Next time confirm when yuh finish!"
    FORGOT_TO_CONFIRM_NOT_YOUR_CHORE = "❌ {mention}, **{chore}** wasn't assigned to yuh last week. Cyan't claim yuh forgot fi confirm something dat wasn't yours!"