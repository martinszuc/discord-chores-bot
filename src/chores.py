...
class ChoresCog(commands.Cog):
    def __init__(self, bot):
        ...
        # Emoji for reaction handling
        self.forgot_confirm_emoji = "🔄"
    ...
    async def send_pre_week_reminder(self, channel=None):
        """Send reminders 1 day before new week starts for incomplete chores."""
        logger.info("Sending pre-week reminders for incomplete chores")
        
        # Get pending chores
        pending_chores = self.schedule_manager.get_pending_chores()
        if not pending_chores:
            logger.info("No pending chores - everyone completed their tasks!")
            if channel:
                await channel.send(BotStrings.PRE_WEEK_REMINDER_ALL_DONE)
            return
        ...
        logger.debug("Sending pre-week reminder header")
        await channel.send(BotStrings.PRE_WEEK_REMINDER_HEADER)
        ...
        for chore in pending_chores:
            ...
            discord_id = flatmate["discord_id"]
            logger.debug(f"Creating pre-week reminder for chore '{chore}' assigned to {flatmate_name} (ID: {discord_id})")
            
            # Create the reminder message
            reminder_message = BotStrings.PRE_WEEK_REMINDER_MESSAGE.format(
                mention=f"<@{discord_id}>",
                chore=chore
            )
            ...
            reminder_count += 1
            logger.debug(f"Sent pre-week reminder for chore '{chore}' to {flatmate_name}")
        
        logger.info(f"Sent {reminder_count} pre-week reminders for incomplete chores")

    async def post_schedule(self, channel=None):
        """Post the weekly chore schedule with penalty system for incomplete tasks."""
        logger.info("Posting new chore schedule with penalty check")
        ...
        # Check for incomplete chores from last week and display penalties
        incomplete_chores = self.schedule_manager.get_pending_chores()
        previous_assignments = self.schedule_manager.get_previous_assignments()
        
        penalty_info = None
        
        if incomplete_chores and previous_assignments:
            logger.info(f"Found {len(incomplete_chores)} incomplete chores from last week")
            
            # Track flatmates who didn't complete their work
            penalty_flatmates = {}  # {flatmate_name: [list of incomplete chores]}
            completed_flatmates = []  # Flatmates who completed their work
            
            # Identify who didn't complete their work
            for chore in incomplete_chores:
                flatmate_name = previous_assignments.get(chore)
                if flatmate_name:
                    if flatmate_name not in penalty_flatmates:
                        penalty_flatmates[flatmate_name] = []
                    penalty_flatmates[flatmate_name].append(chore)
                    logger.info(f"Flatmate {flatmate_name} will receive penalty for not completing {chore}")
            
            # Identify who completed their work (candidates for getting a break)
            for chore, assignee in previous_assignments.items():
                if chore not in incomplete_chores and assignee not in completed_flatmates:
                    completed_flatmates.append(assignee)
                    logger.debug(f"Flatmate {assignee} completed their work")
            
            # Only proceed if we have both penalized and completed flatmates
            if penalty_flatmates and completed_flatmates:
                import random
                
                # Display penalty header
                await channel.send(BotStrings.PENALTY_HEADER)
                
                lucky_flatmates = []
                
                # For each penalized flatmate, pick a lucky person who gets a break
                for penalized_flatmate, incomplete_chores_list in penalty_flatmates.items():
                    # Check if penalized flatmate is still active this week
                    active_flatmates = self.config_manager.get_active_flatmates()
                    is_active = any(f["name"] == penalized_flatmate for f in active_flatmates)
                    
                    if not is_active:
                        logger.debug(f"Skipping penalty for {penalized_flatmate} (not active this week)")
                        continue
                    
                    # Pick a lucky flatmate (who isn't already lucky)
                    available_for_luck = [f for f in completed_flatmates if f not in lucky_flatmates]
                    
                    if available_for_luck:
                        lucky_flatmate = random.choice(available_for_luck)
                        lucky_flatmates.append(lucky_flatmate)
                        logger.info(f"Lucky flatmate {lucky_flatmate} gets a break due to {penalized_flatmate}'s penalty")
                        
                        # Get discord mentions
                        penalized_discord = self.config_manager.get_flatmate_by_name(penalized_flatmate)
                        lucky_discord = self.config_manager.get_flatmate_by_name(lucky_flatmate)
                        
                        if penalized_discord:
                            # Notify penalized flatmate
                            # We'll show the first incomplete chore as example
                            example_chore = incomplete_chores_list[0]
                            penalty_msg = BotStrings.PENALTY_MESSAGE.format(
                                mention=f"<@{penalized_discord['discord_id']}>",
                                chore=example_chore,
                                new_chore="[will be assigned in the new schedule]"
                            )
                            await channel.send(penalty_msg)
                        
                        if lucky_discord:
                            # Notify lucky flatmate
                            lucky_msg = BotStrings.PENALTY_LUCKY_PERSON.format(
                                mention=f"<@{lucky_discord['discord_id']}>"
                            )
                            await channel.send(lucky_msg)
                
                # Store penalty info for the schedule generation
                penalty_info = {
                    "penalized": penalty_flatmates,
                    "lucky": lucky_flatmates
                }
                
                # Store in schedule manager for use during generation
                self.schedule_manager.schedule_data["penalty_info"] = penalty_info
        
        # ============= END PENALTY SYSTEM LOGIC =============
        
        # Generate new schedule (which will now account for penalties via priority scores)
        ...
        for chore, flatmate_name in assignments.items():
            ...
            discord_id = flatmate["discord_id"]
            logger.debug(f"Creating assignment message for chore '{chore}' assigned to {flatmate_name} (ID: {discord_id})")
            ...
            await message.add_reaction(emojis["completed"])
            await message.add_reaction(emojis["unavailable"])
            await message.add_reaction("🔄")  # Forgot to confirm emoji
    ...
    async def on_raw_reaction_add(self, payload):
        ...
        if payload.message_id in self.message_cache:
            ...
            pass
        elif hasattr(self, 'next_week_planning_cache') and self.next_week_planning_cache and payload.message_id == self.next_week_planning_cache.get("message_id"):
        ...
        if emoji_name == emojis["completed"]:
            ...
            if next_flatmate_name:
                ...
                if next_flatmate:
                    ...
                    await new_message.add_reaction(self.forgot_confirm_emoji)
                    ...
                    logger.debug(f"Updated message cache after reassignment")

                    logger.info(f"Chore '{chore}' successfully reassigned from {flatmate['name']} to {next_flatmate_name}")
            else:
                logger.warning(f"Failed to reassign chore '{chore}'")
                await channel.send(BotStrings.ERR_REASSIGN_FAILED.format(chore=chore))

        # Update the on_raw_reaction_add listener to handle the forgot-to-confirm emoji:
        # Add this to the _handle_chore_reaction method after the completed emoji check:
        elif emoji_name == self.forgot_confirm_emoji:
            # Handle "forgot to confirm" reaction
            logger.info(f"Flatmate {flatmate['name']} claiming they forgot to confirm chore '{chore}'")

            # Check if this chore was assigned to them in the previous week
            previous_assignments = self.schedule_manager.get_previous_assignments()

            if not previous_assignments:
                logger.warning("No previous assignments found")
                await message.remove_reaction(payload.emoji, user)
                await channel.send(
                    f"{user.mention} {BotStrings.FORGOT_TO_CONFIRM_TOO_LATE}",
                    delete_after=10
                )
                return
            ...
            if previous_assignee != flatmate["name"]:
                logger.warning(f"{flatmate['name']} tried to claim they forgot to confirm a chore that wasn't theirs")
                await message.remove_reaction(payload.emoji, user)
                await channel.send(
                    BotStrings.FORGOT_TO_CONFIRM_NOT_YOUR_CHORE.format(
                        mention=user.mention,
                        chore=chore
                    ),
                    delete_after=10
            ...
            # Mark the chore as completed retroactively
            logger.info(f"Retroactively marking chore '{chore}' as completed by {flatmate['name']}")

            pending_chores = self.schedule_manager.get_pending_chores()
            if chore in pending_chores:
                self.schedule_manager.schedule_data["pending_chores"].remove(chore)
                self.schedule_manager._save_schedule_data()
                logger.info(f"Removed '{chore}' from pending chores")

            # Update statistics
            self.config_manager.update_flatmate_stats(flatmate["name"], "completed")

            # Send confirmation message
            await channel.send(
                BotStrings.FORGOT_TO_CONFIRM_MESSAGE.format(
                    mention=user.mention,
                    chore=chore
                )
            )
            logger.info(f"Flatmate {flatmate['name']} successfully claimed forgot-to-confirm for '{chore}'")
