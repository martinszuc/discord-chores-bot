...
class ScheduleManager:
    ...
    def generate_new_schedule(self):
        """Generate a new chore schedule with penalty system for incomplete tasks."""
        logger.info("Generating new chore schedule with penalty system")
        ...
        # Check for pending chores (incomplete tasks from last week)
        pending_chores_from_last_week = self.get_pending_chores()
        logger.debug(f"Pending chores from last week: {pending_chores_from_last_week}")
        
        # Track flatmates with penalties and those who get a break
        penalty_flatmates = {}  # {flatmate_name: [list of incomplete chores]}
        completed_flatmates = []  # Flatmates who completed their work
        
        if previous_assignments and pending_chores_from_last_week:
            # Identify who didn't complete their work
            for chore in pending_chores_from_last_week:
                assignee = previous_assignments.get(chore)
                if assignee:
                    if assignee not in penalty_flatmates:
                        penalty_flatmates[assignee] = []
                    penalty_flatmates[assignee].append(chore)
                    logger.info(f"Penalty: {assignee} didn't complete '{chore}'")
            
            # Identify who completed their work (candidates for getting a break)
            for chore, assignee in previous_assignments.items():
                if chore not in pending_chores_from_last_week and assignee not in completed_flatmates:
        ...
        # If there are flatmates with penalties, we need to:
        # 1. Give them extra chores from the pool
        # 2. Give someone who completed their work a break (one less chore or week off)
        
        penalty_extra_chores = {}  # {flatmate_name: [extra_chores]}
        lucky_flatmates = []  # Flatmates who get a break
        
        if penalty_flatmates and completed_flatmates:
            logger.info(f"Applying penalty system: {len(penalty_flatmates)} flatmates with penalties")
            
            # For each flatmate with penalty, assign them one extra chore
            import random
            available_for_luck = completed_flatmates.copy()
            
            for penalized_flatmate, incomplete_chores in penalty_flatmates.items():
                # Only apply penalty if flatmate is still active and not on vacation
                if penalized_flatmate not in [f["name"] for f in flatmates]:
                    logger.debug(f"Skipping penalty for {penalized_flatmate} (not active this week)")
                    continue
                
                # Pick a random flatmate who completed their work to give a break
                if available_for_luck:
                    lucky_flatmate = random.choice(available_for_luck)
                    lucky_flatmates.append(lucky_flatmate)
                    available_for_luck.remove(lucky_flatmate)
                    logger.info(f"Lucky flatmate {lucky_flatmate} gets a break due to {penalized_flatmate}'s penalty")
                
                # Store penalty info for later (will be applied after normal assignment)
                if penalized_flatmate not in penalty_extra_chores:
                    penalty_extra_chores[penalized_flatmate] = []
        
        # Store penalty and lucky info in schedule data for notification during post_schedule
        self.schedule_data["penalty_info"] = {
            "penalized": penalty_flatmates,
            "lucky": lucky_flatmates,
            "extra_chores": penalty_extra_chores
        }
        ...
        for flatmate in flatmates:
            ...
            # PENALTY: Penalized flatmates get higher priority (they get extra work)
            if name in penalty_flatmates:
                priority_score += 50
                logger.debug(f"Flatmate {name} gets penalty priority boost")
            
            # LUCKY: Lucky flatmates get lower priority (they get a break)
            if name in lucky_flatmates:
                priority_score -= 100
                logger.debug(f"Flatmate {name} gets lucky break priority reduction")
            
            logger.debug(f"Flatmate {name} priority score: {priority_score} (completed: {completed}, skipped: {skipped})")
        ...
        # Track how many chores each flatmate gets
        ...
        for chore in eligible_chores:
            ...
            for flatmate in available_flatmates[:]:
                # Skip lucky flatmates for first pass (they get fewer chores)
                if flatmate["name"] in lucky_flatmates and flatmate_chore_count[flatmate["name"]] > 0:
                    logger.debug(f"Skipping {flatmate['name']} - lucky flatmate already has a chore")
                    continue
                
                if flatmate["name"] != previous_assignee:
                    new_assignments[chore] = flatmate["name"]
                    flatmate_chore_count[flatmate["name"]] += 1
                    
                    # Remove from available if lucky (they only get 0-1 chores)
                    if flatmate["name"] in lucky_flatmates:
                        available_flatmates.remove(flatmate)
                        logger.info(f"Assigned '{chore}' to lucky {flatmate['name']} (their only task)")
                    else:
                        # Only remove non-lucky flatmates after they get a task
                        if flatmate_chore_count[flatmate["name"]] >= 1 and flatmate["name"] not in penalty_flatmates:
                            available_flatmates.remove(flatmate)
            ...
            if not assigned and available_flatmates:
                ...
                if flatmate["name"] in lucky_flatmates or (flatmate["name"] not in penalty_flatmates and flatmate_chore_count[flatmate["name"]] >= 1):
                    available_flatmates.remove(flatmate)
        ...
        if remaining_chores:
            ...
            # Prioritize flatmates with fewest assignments (especially penalized ones)
            available_for_extra = [(f, flatmate_chore_count[f["name"]]) for f in sorted_flatmates if f["name"] not in lucky_flatmates]
        ...
        logger.info(f"Chore counts: {flatmate_chore_count}")
